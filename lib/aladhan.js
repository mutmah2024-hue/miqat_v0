...
/*
 * Get the Qibla direction from AlAdhan.
 *
 * AlAdhan returns the direction of the
 * Ka'bah as a bearing in degrees.
 */
...
export async function getQiblaDirection(
	...
	if (!response.ok) {
		throw new Error(
			"Unable to fetch Qibla direction."
		);
	}
	...
	if (
		result.code !== 200 ||
		typeof result.data?.direction !== "number"
	) {
		throw new Error(
			"AlAdhan could not return the Qibla direction."
		);
	}
