...
export default function Dashboard() {
	...
	useEffect(() => {
		...
		async function loadTodayProgress() {
			try {
				const [quranToday, quranSettings, focusToday, prayerToday] =
					await Promise.all([
						getTodayQuranProgress(user.uid),
						getQuranSettings(user.uid),
						getTodayFocusStats(user.uid),
						getTodayPrayerProgress(user.uid),
					]);

				setQuranProgress({
					pagesRead: quranToday.pagesRead ?? 0,
					dailyGoal: quranSettings.dailyGoal ?? 5,
				});

				setFocusProgress({
					sessions: focusToday.sessions ?? 0,
					focusTime: focusToday.focusTime ?? 0,
				});

				setPrayerProgress(prayerToday);
			} catch (error) {
				console.error("Could not load today's progress:", error);
			}
		}
			...
			try {
				...
				getPrayerTimes(location.latitude, location.longitude)
					...
					.catch((error) => {
						...
						setLocationError("We couldn't load your prayer times.");
				...
				localStorage.removeItem("miqat_location");
			}
		}
		...
		navigator.geolocation.getCurrentPosition(
			...
			(error) => {
				...
				if (error.code === 1) {
					...
					setLocationError("We couldn't determine your location.");
				}
			},
		);
	}, [user]);
	...
	const handleLocationSelect = async (location) => {
		...
		localStorage.setItem("miqat_location", JSON.stringify(location));
		...
		try {
			...
			setLocationError("We couldn't load prayer times for this location.");
		} finally {
			setPrayerLoading(false);
		}
	};
	...
	const completedPrayerCount =
		Object.values(prayerProgress).filter(Boolean).length;
					...
					<div className="mx-auto max-w-7xl px-6 py-10 lg:px-10 lg:py-14">
									...
									<select
										...
										onChange={(event) => {
											const selected = locations.find(
												(location) => location.state === event.target.value,
											);
						...
						<section className="mt-6 grid gap-6 md:grid-cols-2">
							...
							<article className="rounded-3xl border border-border bg-surface p-7">
								...
								>
									<a href="../quran">Continue reading</a>
								</button>
							</article>
						...
						<section className="mt-6 rounded-3xl border border-border bg-surface p-7 sm:p-8">
							...
							<div className="mt-8 grid grid-cols-2 gap-6 sm:grid-cols-4">
								<ProgressItem
									label="Prayer"
									value={`${completedPrayerCount} / 5`}
								/>

								<ProgressItem
									label="Qur'an"
									value={`${quranProgress.pagesRead} / ${quranProgress.dailyGoal} pages`}
								/>

								<ProgressItem
									label="Focus"
									value={`${focusProgress.focusTime} min`}
								/>

								<ProgressItem label="Reflection" value="—" />
							</div>
						</section>
					</div>
				</div>
