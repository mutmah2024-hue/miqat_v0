...
export default function Focus() {
	...
	useEffect(() => {
		...
		async function loadSettings() {
			try {
				const settings = await getFocusSettings(user.uid);

				if (settings) {
					setFocusMinutes(settings.focusDuration);

					setBreakMinutes(settings.breakDuration);

					setDailyGoal(settings.focusGoal);

					setTimeLeft(settings.focusDuration * 60);
				}
			} catch (error) {
				console.error("Could not load Focus settings:", error);
			}
		}
	...
	useEffect(() => {
		...
		async function loadTodayStats() {
			try {
				...
				console.error("Could not load Focus statistics:", error);
			} finally {
	...
	useEffect(() => {
		...
		async function loadFocusHistory() {
			try {
				const history = await getFocusHistory(user.uid);

				setFocusHistory(history);
			} catch (error) {
				console.error("Could not load Focus history:", error);
			}
		}
			...
			setTimeLeft((previous) => {
				if (previous <= 1) {
					...
					if (mode === "focus") {
						...
						saveCompletedFocusSession(user.uid, focusMinutes)
							.then((updatedStats) => {
								setSessions(updatedStats.sessions);

								setFocusTime(updatedStats.focusTime);
								...
								return getFocusHistory(user.uid);
							})
							.then((history) => {
								setFocusHistory(history);
							...
							.catch((error) => {
								console.error("Could not save completed Focus session:", error);
							})
							.finally(() => {
	...
	}, [isRunning, mode, focusMinutes, breakMinutes, user]);
	...
	const saveSettings = async (newFocusMinutes, newBreakMinutes, newGoal) => {
		if (!user) return;

		try {
			await saveFocusSettings(user.uid, {
				focusDuration: newFocusMinutes ?? focusMinutes,

				breakDuration: newBreakMinutes ?? breakMinutes,

				focusGoal: newGoal ?? dailyGoal,
			});
		} catch (error) {
			console.error("Could not save Focus settings:", error);
		}
	};
	...
	const handleFocusChange = (minutes) => {
		...
		saveSettings(minutes, null, null);
	};
	...
	const handleBreakChange = (minutes) => {
		...
		saveSettings(null, minutes, null);
	...
	const handleGoalChange = (minutes) => {
		...
		saveSettings(null, null, minutes);
	};
	...
	const progress = Math.min((focusTime / dailyGoal) * 100, 100);
								...
								<div className="mt-5 flex flex-wrap gap-2">
									{[25, 45, 60].map((minutes) => (
										<button
											key={minutes}
											type="button"
											onClick={() => handleFocusChange(minutes)}
								...
								<div className="mt-5 flex flex-wrap gap-2">
									{[5, 10, 15].map((minutes) => (
										<button
											key={minutes}
											type="button"
											onClick={() => handleBreakChange(minutes)}
											className={`rounded-xl px-4 py-2.5 text-sm font-medium transition-colors ${
